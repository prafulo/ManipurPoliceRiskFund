...
export default function SubscriptionReleaseReportPage() {
  ...
  const expiredReleaseAmount = 50000;

  useEffect(() => {
    async function loadInitialData() {
        try {
            const { members, payments } = await fetchData();
    ...
    const data: ReportRow[] = closedMembers.map(member => {
      ...
      if (member.closureReason === 'Death') {
        ...
        totalMonthsPaid = memberPayments.reduce((sum, p) => {
            try {
                const monthsArray = typeof p.months === 'string' ? JSON.parse(p.months) : p.months;
                return sum + (Array.isArray(monthsArray) ? monthsArray.length : 0);
            } catch {
                return sum;
            }
        }, 0);
  ...
  }, [dateRange, allMembers, allPayments, expiredReleaseAmount, dataLoading]);
